...
async function willFail(promise, message) {
    try {
        await promise;
        throw null;
    } catch (error) {
        assert(error, "Expected an error but did not get one");
        assert(error.message.indexOf(message) >= 0, "Expected an error with '" + message + "' but got '" + error.message + "' instead");
    }
}
...
module.exports = {
    ...
    willFail: async function (promise, message) {
        await willFail(promise, message);
    },
};
